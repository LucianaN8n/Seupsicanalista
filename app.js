(function(){
  ...
  function refreshPerguntas(){
    ...
    const qs = pick3(base);
    listaPerguntas.innerHTML = qs.map(q=>`<li>${esc(q)}</li>`).join('');
    perguntasCard.classList.remove('hidden');
  }
  $('#queixas')?.addEventListener('blur', refreshPerguntas);
  $('#queixas')?.addEventListener('change', refreshPerguntas);
  $('#queixas')?.addEventListener('keyup', (e)=>{ if(e.key==='Enter'){ refreshPerguntas(); } });
  ...
  function buildHTML(data, tipo='paciente'){
    ...
    if(obs && obsIncluir){
      html += `<h3 class="section-title">Observações do terapeuta</h3><blockquote>${nl2br(esc(obs))}</blockquote>`;
    }

    // Escalas (monitoramento)
    if(gad7 || phq9){
      html += `<h3 class="section-title">Escalas (monitoramento)</h3><ul>`;
      if(gad7){ html += `<li>GAD-7: ${gad7.sum} (${esc(gad7.class)}) — respondidos: ${gad7.answered}/7</li>`; }
      if(phq9){ html += `<li>PHQ-9: ${phq9.sum} (${esc(phq9.class)}) — respondidos: ${phq9.answered}/9</li>`; }
    ...
    if(tipo==='supervisao'){
      ...
      if(hyp.length){ html += `<h3 class="section-title">Hipóteses clínicas iniciais</h3><ul>` + hyp.map(h=>`<li>${esc(h)}</li>`).join('') + `</ul>`; }
    ...
    // Segurança (comum)
    html += `<h3 class="section-title">Segurança & Encaminhamento</h3><p>${esc(risk)}</p><ul><li>Encaminhar para avaliação médica/psiquiátrica quando aplicável.</li></ul>`;
  ...
  form?.addEventListener('submit', (e)=>{
    ...
    // Por padrão renderiza a versão "paciente"; a de supervisão pode ser gerada no botão próprio
    resultadoEl.innerHTML = buildHTML(data, 'paciente');
    resultadoSection.classList.remove('hidden');
    resultadoSection.scrollIntoView({behavior:'smooth', block:'start'});

    // cache simples (para PDF de supervisão)
  ...
  $('#btnPdfPaciente')?.addEventListener('click', async ()=>{
    await exportPDF(resultadoEl, `relatorio_paciente_v4.pdf`);
  });
  $('#btnPdfSupervisao')?.addEventListener('click', async ()=>{
    const data = window.__lastData || {};
    const html = buildHTML(data, 'supervisao');
    const holder = document.createElement('div');
    holder.style.position='fixed'; holder.style.left='-9999px'; holder.style.top='0';
    holder.innerHTML = `<div class="resultado">${html}</div>`;
    document.body.appendChild(holder);
    await exportPDF(holder, `relatorio_supervisao_v4.pdf`);
    document.body.removeChild(holder);
  });

  // ====== Modo Treino — Protótipo simples
  const exemplos = [
  ...
  $('#btnCaso')?.addEventListener('click', ()=>{
    ...
    $('#casoOut').textContent = `Nome: ${c.nome} (${c.idade})\nQueixa: ${c.queixa}\nExtra: ${c.extra}`;
  });
  $('#btnCasoFeedback')?.addEventListener('click', ()=>{
  ...
  function init(){
    ...
    if(gadGrid || phqGrid){
      ...
      const build = (prefix, items)=>{
        const grid = document.querySelector(`[data-scale="${prefix}"]`);
        if(!grid) return;
        items.forEach((label, i)=> grid.insertAdjacentHTML('beforeend', `<div class="scale-item"><div style="flex:1">${label}</div><div>${[0,1,2,3].map(v=>`<label><input type="radio" name="${prefix}_${i}" value="${v}"> ${v}</label>`).join(' ')}</div></div>`));
      };
      build('gad7', gad7Items);
      build('phq9', phq9Items);
    }
  }
